...
| `llmService.ts` | `queryJobs(question)` — builds a text context from `getMergedJobs()` (non-shipped jobs, plus any blocked shipped ones, capped at 150 to bound the prompt; job number, description, customer, PM, **Materials Manager**, status, blocked flag/reason, ship date, last 5 notes) and POSTs it with the question to a local Ollama server's `/api/generate` (`OLLAMA_BASE_URL` / `OLLAMA_MODEL` in `.env`, default `qwen3:14b`). The prompt also states **today's real date** (server clock) so "this week" / "overdue" / "next month" questions have ground truth instead of the model guessing. Every call — success or failure — goes through `logNetworkRequest()` so it's visible in the Monitoring panel and the security report. 60s timeout; throws a typed `LlmError` (`empty_question`, `question_too_long`, `llm_unreachable`, `llm_error`, `llm_bad_response`) that the route maps to the right HTTP status. Answers can vary run-to-run for the same question (model sampling) — this is expected, not a bug. |
