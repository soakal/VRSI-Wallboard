...
| `llm.ts` | `POST /api/llm/query` — natural-language job questions, answered by a **local** Ollama server on the homelab LAN (never a cloud endpoint). Admin-token gated like the rest of the board API; every call is logged. Returns 400 for an empty/too-long question, 502 if the LLM can't be reached. |

### Services (`src/services/`) — the business logic

| File | What it does and why |
|------|----------------------|
| `boardService.ts` | The heart of the board. `getMergedJobs()` combines the imported spreadsheet rows with the saved per-job state (status, overrides, notes) into the `BoardJob` objects the UI shows — including `isSpare` (denormalized `isSpareJob()` result) and `blockedAt` (when the manual block was set), both added for `llmService.ts` but available to any consumer. `getDerivedUsers()` builds the user picker from the PM and Materials Manager columns of **non-shipped** jobs (people with only shipped jobs disappear — no clutter), plus configured super users and manual extras. `isSpareJob()` / `getJobBoardTab()` decide which tab owns a job (spare-parts PM match or `SP-` job numbers). `formatJobPmLabel()` turns a stored (lowercased, possibly-email) PM/MM string into a display name. `deepMergeConfig` merges config updates field-by-field so a partial save can never wipe other settings. |
...
| `supportService.ts` | In-app Support reports (v1.1.6). `buildSupportBundle()` assembles `message.txt`, `system-info.txt`, and optional log tails into a zip (Desktop copy + archive under `logs\support-reports\`). `composeSupportMail()` spawns `Open-SupportMail.ps1` exactly once (Subject and Body via temp files, never argv); the script reports `outlook` or `mailto` on stdout, and its stderr (the real COM error) is logged on failure. `DEFAULT_SUPPORT_EMAIL` / `SUPPORT_EMAIL` in `.env` — never returned to the client. |
| `llmService.ts` | `queryJobs(question)` — builds a text context from `getMergedJobs()` and POSTs it with the question to a local Ollama server's `/api/generate` (`OLLAMA_BASE_URL` / `OLLAMA_MODEL` in `.env`, default `qwen3:14b`). `buildContext()` filters to non-shipped jobs (plus any blocked shipped ones), **sorts soonest-ship-date first** so the 150-job cap drops the least urgent jobs rather than an arbitrary spreadsheet-order tail, and prepends a `buildSummary()` block — server-computed totals/per-status counts/blocked/overdue counts over the FULL (unsliced) list, so "how many" questions use real arithmetic instead of the model counting a long list itself (a known LLM failure mode). `formatJobForContext()` emits job number, description, customer, spare-vs-project `type` (via `isSpareJob`), PM/MM (humanized via `formatJobPmLabel`), human-readable status, NEW / new-note flags, blocked + reason + since-date, ship date annotated with a precomputed `(in N days)` / `(N days OVERDUE)` (`daysUntil`/`formatShipDate` — date math happens here, not in the model), ship-date-override history when changed, ship-to-PM and PABS-complete dates, binder-printed (omitted for spares), and the 5 most recent notes with dates. The prompt also states **today's real date** (server clock) and explicitly tells the model shipped jobs are excluded and not to confuse "ship date" with "ship to PM." Every LLM call — success or failure — goes through `logNetworkRequest()` so it's visible in the Monitoring panel and the security report. 60s timeout; throws a typed `LlmError` (`empty_question`, `question_too_long`, `llm_unreachable`, `llm_error`, `llm_bad_response`) that the route maps to the right HTTP status. Answers can still vary run-to-run for the same question (model sampling) — expected, not a bug. |
...
| `AskPanel.tsx` | Slide-over opened by **Ctrl+J** (or Escape to close, like the other panels). Free-text question + a few example-question shortcuts, calls `askJobQuestion()` (`api/llmApi.ts` → `POST /api/llm/query`) and renders the local LLM's plain-text answer or an error. Answered entirely by the on-LAN Ollama server — no cloud call. |
...
| `Open-SupportMail.ps1` | Called by the server for in-app Support (single invocation — both attempts happen inside the script). Tries classic Outlook COM with the diagnostic zip attached (`.Display()` — user reviews and clicks Send), **bounded by its own 10s watchdog runspace** — on new-Outlook-only kiosks the COM activation was seen *hanging* instead of failing, which used to eat the server's whole 30s spawn budget so the fallback never ran and nothing opened. If COM fails or hangs **before** `Display()` was attempted it launches `mailto:` with To + Subject (no `&body=` — new Outlook's handler was seen decoding a subject+body query string into the To field, making the mail unsendable; subject alone was verified live to populate cleanly with no garbling); a failure/hang **at or after** `Display()` fails closed (never stacks mailto on a window that may be visible). Prints the method used on stdout; writes the real COM error/timeout to stderr; hard-exits via `[Environment]::Exit` (an abandoned hung COM thread is a foreground thread that would otherwise keep powershell.exe alive past the server's timeout). Email address passed from server only — never from the browser. |
