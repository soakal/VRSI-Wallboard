...
| `Open-SupportMail.ps1` | Called by the server for in-app Support (single invocation — both attempts happen inside the script). Tries classic Outlook COM with the diagnostic zip attached (`.Display()` — user reviews and clicks Send); if COM fails **before** `Display()` was attempted it launches a recipient-only `mailto:` (no `?subject=&body=` — new Outlook's handler was seen decoding those into the To field, making the mail unsendable). Prints the method used on stdout; writes the real COM error to stderr. Email address passed from server only — never from the browser. |
