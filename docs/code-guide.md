...
| `update.ts` | `GET /api/update/check` — compares the running version (from `server/package.json`) against the latest GitHub release, cached 6 h so kiosks don't hammer GitHub. `POST /api/update/run` — launches the self-update script detached (it survives the server being stopped mid-update). Picks `Update-WallBoard.ps1` when a `.git` folder exists (dev machine), otherwise `Update-FromRelease.ps1` (kiosk). **Critical:** uses `path.basename(script)` for the PowerShell `-File` arg (not the absolute path) because PowerShell 5.1 does not reliably set `$PSScriptRoot` when spawned non-interactively with a path containing spaces — which would silently break the dot-source of `_common.ps1` before any log is written. Captures stderr for 10 s so early startup failures surface in server logs. |
...
| `storage.ts` | Backup/restore/audit endpoints used by the System panel (Ctrl+M). |
...
| `hooks/useEvents.ts` | Fetches `/api/events` from the **1st of the month** through today+45 days, in every view mode. Why from the 1st: the agenda shows past-due ship dates from earlier in the month — if they aren't fetched, they can't be shown. |
...
| `BoardHeader.tsx` | Logo, the user switcher, a ⚙ Settings button (so Settings is reachable from the board on mobile), and the three tabs: Project / Spare Parts / Archive (Users and Import moved into Settings → Board). Tab clicks and user switches are guarded by `confirmDiscardUnsaved()`. |
...
|--------|----------------------|
| `Start-TrayApp.ps1` | The supervisor. Shows the blue **W** tray icon (drawn in code — no icon file to lose), starts the Node server, and watches it: crash → auto-restart, max 3 per minute, then a warning balloon instead of a restart loop. Detects port squatters (something else on 3001) instead of crash-looping on EADDRINUSE. Refreshes PATH from the registry because Task Scheduler doesn't load user PATH — without this, winget-installed Node is invisible at logon. |
...
| `Update-FromRelease.ps1` | Kiosk self-update: download the latest GitHub release zip, verify it, stop tray+server, copy over the install (data and `.env` untouched), `npm install`, restart everything including the kiosk browser. Logs to `update.log`. This is what the Settings Update button runs. |
| `Update-WallBoard.ps1` | Dev-machine equivalent: `git pull` + rebuild + restart. `-Unattended` skips its prompts when launched by the server. |
...
| `Install-WallBoard.ps1` / `Uninstall-WallBoard.ps1` | Create data dirs, generate a random `ADMIN_TOKEN`, build, optionally register startup/backup tasks — and cleanly remove it all. |
