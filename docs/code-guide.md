...
| `Update-FromRelease.ps1` | Kiosk self-update: download the latest GitHub release zip, verify it, stop tray+server, copy over the install (data and `.env` untouched), `npm install`, restart everything including the kiosk browser. Logs to `update.log`. This is what the Settings Update button runs (launched via WMI — see `update.ts` above for why). Has a `$PSScriptRoot` fallback guard at the top as insurance for unusual invocation contexts. |
