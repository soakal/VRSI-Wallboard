...
| `GET` | `/api/storage/support-info` | Support form limits (inbox not exposed) |
| `POST` | `/api/storage/support` | Build support zip; open Outlook or mailto (server-side) |
| `GET` | `/api/storage/support-download/:filename` | Download archived support zip (mailto fallback) |
...
| `SUPPORT_EMAIL` | No | `briank@vrs-inc.com` | Server-only mailto/Outlook destination — never shown in UI |
...
| 2026-07-15 | Fable+Claude | v1.1.8: Support-mail garbling fix. Brian reported a live-kiosk screenshot: Send Support opened Outlook with an empty Subject, correct Body, and To containing the decoded tail of a `subject=&body=` query string — unsendable. Root cause: `composeSupportMail()` fired the `outlook` attempt then, on any failure (silently swallowed by the old script's bare `catch{exit 1}`), fired a *second, separate* `mailto` attempt whose `subject=&body=` query string the new-Outlook (`olk.exe`) mailto handler mis-parsed into the To field. Fix: `Open-SupportMail.ps1` rewritten to a single invocation — COM first, mailto fallback only if `.Display()` was never reached, and the fallback is now recipient-only (no query string, the demonstrated garbling vector); Subject now travels via a temp file like Body already did; real COM errors go to stderr instead of being swallowed. `supportService.ts` updated to match (one spawn instead of two, logs script stderr). Build/tests/typecheck clean (63/63). Real-Outlook behavior NOT verifiable from a dev/CI environment — an attempted local test was a false negative (the Tray scheduled task auto-restarted the OLD installed copy the instant the dev server's port freed up) — live verification against the updated installed copy is still outstanding; see `docs/ai-memory.md` for the checklist. | Support feature was broken end-to-end for real users on the live kiosk |
| 2026-07-15 | Fable+Claude | v1.1.9: Support-mail COM-hang fix. The v1.1.8 single-invocation design regressed on the same new-Outlook-only kiosk — `New-Object -ComObject Outlook.Application` HANGS there (doesn't fail fast), and with the mailto fallback now sequentially after it in the same process, the hang ate the whole 30s `SUPPORT_SPAWN_TIMEOUT_MS` so Node killed powershell.exe before the fallback ever ran (`combined.log`: `spawnSync powershell.exe ETIMEDOUT`, nothing opened at all). Fix inside `Open-SupportMail.ps1` only: the COM attempt now runs in an in-process STA runspace with its own 10s inner timeout (`BeginInvoke` + `AsyncWaitHandle.WaitOne`; runspace chosen over `Start-Job` — no second powershell startup eating the budget, and a `[hashtable]::Synchronized` shared state keeps the `$displayAttempted` guard readable in real time even while COM is hung). On timeout the runspace is abandoned (no `Stop()`/`Dispose()` — both can block on a thread stuck in native COM) and the same-process mailto fallback runs. Harness on this machine exposed a second pitfall: the abandoned pipeline thread is a *foreground* thread, so plain `exit` left powershell.exe alive for minutes (spawnSync would still report ETIMEDOUT even after mailto opened) — all post-runspace exits now go through `Exit-Hard` (flush stdout/stderr + `[Environment]::Exit`). No `Stop-Process` on OUTLOOK.EXE (can't distinguish a half-initialized automation instance from the user's real session). Outer 30s unchanged (~10s COM + ~2s mailto + startup fits comfortably). Verified: build clean, 63/63 tests, tsc clean, ps1 parses under PS 5.1 (0 errors), and — beyond Fable's harnesses — a direct live smoke test on the actual hanging machine: real run completed in 13.3s with exit 0, stdout `mailto`, stderr correctly reporting the 10s COM timeout (not ETIMEDOUT), and a genuine new-Outlook "New mail" window opened with a clean, ungarbled To field (screenshot-confirmed). | v1.1.8 fixed the garbling but starved its own fallback on machines where COM hangs instead of throwing — Send opened nothing at all |
| 2026-07-15 | BK+Claude | v1.1.10: restore the mailto fallback's Subject line. Brian tested v1.1.9 live and noted the fallback mail opened with no subject — expected, since v1.1.8 had stripped `?subject=&body=` entirely to kill the garbling bug. Live-tested directly on the same hanging-COM machine whether *subject alone* (no body) triggers the same garbling: it does not — `mailto:$To?subject=$encoded` populated both To and Subject cleanly (screenshot-confirmed, twice, including through the actual script path). `Open-SupportMail.ps1`'s fallback now sends `?subject=` only; `&body=` stays out (untested, and the full message is already in the zip's `message.txt` for the manual-attach flow anyway). Comments updated in `supportService.ts` and `docs/code-guide.md` to match. Verified: build clean, 63/63 tests, ps1 parses (0 errors), live smoke test 13.3s/exit 0, window title showed the correct subject text. | Brian: fallback email had no subject; verified a safe way to bring it back |
| 2026-07-15 | BK+Claude | v1.1.11: stop the redundant browser "Save As" prompt on the mailto fallback path. `MonitoringPanel.tsx`'s `handleSendSupport` called `downloadSupportPackage()` unconditionally whenever `method === 'mailto'`, even though `buildSupportBundle()` had already silently copied the zip to the user's Desktop (`result.savedPath` truthy) — so the user got the file twice: once auto-saved, once via an unwanted save dialog. Fix: only trigger the browser download when there's no Desktop copy to point at (`!result.savedPath`); the success message no longer claims a redundant "also downloaded to your browser Downloads folder" when a Desktop copy already exists. Build clean, 63/63 tests. | Brian: don't prompt to save somewhere else when it's already auto-saving to the Desktop |
| 2026-08-31 | BK+Claude | v1.1.16: tray icon shows its version. `scripts/windows/Start-TrayApp.ps1`'s `NotifyIcon` tooltip and right-click menu had never shown a version number in any release — tooltip was hardcoded `'VRSI WallBoard'` and the menu only had Open/Restart/Exit. Now reads `server\package.json`'s `version` field at startup and shows `VRSI WallBoard v1.1.16` as both the tray hover tooltip and a disabled label at the top of the right-click menu. Parse-validated 0 errors under PS 5.1. | Brian noticed the tray icon didn't show a version number even though the app's own Settings page does |
| 2026-08-31 | Fable+Claude | v1.1.15: version-only bump, no code changes. Published solely to give the fixed `Update-FromRelease.ps1` (shipped in v1.1.14, live-verified as reproducing the OLD known bugs on the v1.1.13→v1.1.14 run since that run executed v1.1.13's pre-fix script) a genuine "update available" scenario to run against — v1.1.14 was both current and latest after that run, so the UI hid the Update button and the fix stayed untested. This release exists purely to close that loop. | Closing out live verification of the v1.1.14 update-mechanism fix, which needs an actual newer release to run against |
| 2026-08-31 | Fable+Claude | v1.1.14: fixed two real bugs found by the first-ever LIVE update-cycle test (v1.1.12 → v1.1.13, on a real INSTALL.bat kiosk). `scripts/windows/Update-FromRelease.ps1`: (1) `update-status.json` always had empty `fromVersion`/`toVersion` — no caller passed them to `Write-UpdateStatus` (which already supported them); now captures the pre-update version from `release-info.json` before the copy step and the target from the release tag. (2) The `finally` block warned "Could not re-enable tray task: Access is denied" on every unelevated update — the elevated-installer-registered task also rejects `Disable-ScheduledTask` from the standard user, so `$taskDisabled` was set `true` without checking; now verified against the task's actual post-disable state, and the finally block only re-enables/warns when the task is genuinely `Disabled`. Parse-validated 0 errors under PS 5.1; **this fix itself needs the v1.1.13→v1.1.14 update run to be considered live-verified**, per the project's standing rule that PowerShell/Windows changes aren't trusted until actually executed. | Live-testing the updater (never done before this session) found it lying about which version it updated from/to, and emitting a false-alarm warning on every real kiosk |
| 2026-08-31 | Fable+Claude | v1.1.13: note timestamps always show the posted date. `NotesSection.tsx` note-header timestamp previously showed only relative time (`formatDistanceToNow`, e.g. "3 weeks ago") with no absolute date — once a note aged past a couple of days there was no way to tell which calendar day it was actually posted. Now shows `format(noteDate, 'MMM d, yyyy')` (matches `JobCard.tsx`'s ship-date format convention) with the relative time kept as a dimmer parenthetical; "(edited)" suffix unchanged. Client-only, Tailwind-only, no new deps. Verified: `npm run build` clean, `npm test --prefix server` 63/63. | Brian: notes need to always show the actual date posted, not just relative time |
| 2026-08-31 | BK+Claude | v1.1.12 packaged and released to GitHub (https://github.com/soakal/VRSI-Wallboard/releases/tag/v1.1.12) — the audit-remediation code had been merged to `main` on 2026-08-01 (PR #4) but never packaged. This machine had no local Node toolchain until this session; installed Node 24.19.0 LTS via winget, cleared a stale/broken `node_modules` state (missing `@vrsi/wallboard-shared` link, `better-sqlite3` native build blocked by npm 11's install-script gate) via `npm approve-scripts --all` + reinstall, verified `npm run build` clean and `npm test --prefix server` 63/63, then ran `Package-Release.ps1` and `gh release create`. Pruned local `releases/` to the 2 newest (v1.1.11, v1.1.12). | Closing the gap between "code merged" and "release actually published" |
| 2026-08-01 | Claude | v1.1.12: closed the stale audit PR #1 (`docs(audit): full application audit`, opened 2026-07-03 against v1.1.3, never merged — base a month behind `main`, direct merge would have reverted v1.1.4–v1.1.11). Re-verified its 4 HIGH findings against current `main`: all four were still live, a month later. Fixed and ported forward: **H1** `POST /api/board/import`'s file-upload branch now 400s (`no_valid_jobs`) on a zero-job parse before calling `applyBoardImport`, mirroring the guard the JSON-paste branch already had — previously a wrong-file upload could `DELETE FROM jobs` + prune every note-less board-state row with no warning. **H2** `Register-BackupTask.ps1` now registers the backup task under `New-ScheduledTaskPrincipal -UserId <consoleUser> -LogonType Interactive -RunLevel Limited` instead of `-RunLevel Highest`; the kiosk user already has Modify on the install tree (for self-update) and Modify on the data dirs (`Install-DataDirs.ps1`), so an elevated task pointed at a kiosk-writable script was a local privilege-escalation path — closed without narrowing the updater's `icacls` grant. **H3** `AuthSetup`'s `onAuthenticated` prop is now a stable `React.useCallback` (`handleAuthenticated` in `App.tsx`) instead of a new inline arrow every render, which previously tore down and restarted the device-code poll on every 3s auth tick and 409-locked the sign-in screen. **H4** the `/` and `/setup` routes now render off a new debounced `routeAuthenticated` state (only flips false after `unauthCountRef >= 4 || needsReauth`, same threshold the imperative `navigate()` already used) instead of the raw per-poll `isAuthenticated` value, so one transient unauthenticated poll no longer bounces a signed-in kiosk into a fresh device-code flow. `docs/audit-2026-07-03.md` landed on `main` for the first time (was only ever on the unmerged PR branch) with a dated update note correcting its "fixed in that session" claim — those fixes lived on the PR branch only and never reached `main` until now. MEDIUM/LOW findings from the same report were **not** re-verified this session (flagged unconfirmed in the doc); `M5` (multer 1.x) is confirmed still present via the `npm install` deprecation warning. Verified: `npm run build` clean, `npm test --prefix server` 63/63; the two edited `.ps1` files could not be parse-validated locally (no `pwsh` in this Linux session) — reviewed by hand against the working `_Register-Startup.ps1` pattern this mirrors; CI's `ps-lint` job will parse-validate on push. | User: "a security and ops audit sitting unreviewed for a month is a blind spot, not a backlog item — block time, read it, make a call, close the loop" |
| 2026-09-03 | Fable+Claude | v1.1.17: (1) Projects search now returns matches across all 4 tabs (Project/Spare Parts/Archive/Blocked) instead of only the currently-open tab, labeling any result that lives outside the current tab ("In Archive", etc.) — `JobListView.tsx` searches the full unfiltered job list once a query is committed; `boardColors.ts` gains `jobTabOf`/`TAB_LABELS`. (2) Fixed `personIdentity.ts` crashing the Vite dev server (`process is not defined`) — it's aliased into the client bundle and read `process.env.PERSON_ALIASES` unguarded at module load; production was unaffected (Vite's build step stubs `process.env` to `{}`) but local dev was fully blocked. Also this session: cleaned up an abandoned `claude/vrsi-wallboard-local-llm-g6ytqy` branch/PR #5 whose closed-PR diff had leaked a real `ADMIN_TOKEN` value (repo was made private for it) — token rotated, repo made public again, which also fixed the kiosk's unauthenticated `releases/latest` update check (was 404ing against the private repo). Verified: `tsc --noEmit` clean (client+server), notes add/edit/delete confirmed working live on this PC post v1.1.16 update, global search verified live in dev mode (374-result cross-tab match + exact-match archived-job lookup from the Project tab). | Brian: "we want the search to be global search so can search on any page and will give me results"; also cleaning up fallout from an abandoned branch that had made the repo private |
| 2026-09-21 | BK+Claude | v1.1.18: write failures are now visible — SQLITE_READONLY/CANTOPEN/PERM/FULL/IOERR return a `db_unwritable` message ("can't write to its data folder ... NOT saved") instead of a bare Internal Server Error; failed audit-log writes now log a rate-limited warning instead of being swallowed; the startup probe opens `wallboard.db` and `combined.log` read-write (the old `accessSync(W_OK)` cannot see Windows ACLs). Operations guide §4.6 added. Triggered by a kiosk whose server lost write access to `ProgramDataVRSIWallBoard` and failed every save silently for ~12 days. | Brian: "anytime he clicks on shipped or anything on that line and then saves it says it cant save" |
