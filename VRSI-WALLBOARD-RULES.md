...
| 2026-07-03 | BK+AI | Full four-track application audit (`docs/audit-2026-07-03.md`) + remediation. Fixed all 4 HIGH: import guard against board-wipe on an empty/unparseable upload (`board.ts`); backup Task Scheduler job now runs as the LIMITED kiosk user (was `-RunLevel Highest` — a local EoP given the install-tree Modify grant); device-code auth flow starts once via a ref (was 409-storming on every poll) and the router now switches on a debounced auth value (a single transient unauth poll no longer bounces the kiosk into a new device-code flow). Fixed MEDIUM: Graph siteId/driveId validation + encodeURIComponent; Graph read routes (`/api/events`,`/api/calendars`,`/api/sharepoint/*`) now behind `requireAdminToken` when `TRUST_LOCALHOST=false`; refuse to start on a non-loopback `BIND_HOST` with trust on and no `ADMIN_TOKEN`; `multer` 1.x→2.x; restore `conflictStrategy` ('backup'/'live') so a conflicted backup isn't permanently unrestorable; import is now one SQLite transaction; a cleared spreadsheet note now removes the stale Ops Schedule note (§7.4, +3 tests); Apply-all errors surfaced on the card; discard-guard on search/filter toggles; 3 AM watchdog health-probes before reload + event window rolls at midnight; `{data}`/`{error}` envelope on the auth/calendar/sharepoint routes + client `unwrap`; uninstall honors DATA_DIR/BACKUP_DIR/LOGS_DIR overrides; git updater rolls back to the pre-pull commit on a failed build; removed a duplicate Ctrl+S handler. Plus LOWs (pin `react-big-calendar` 1.13.1 — it had drifted to 1.20.0; validate persisted role; shortcut guard for SELECT/contentEditable; poll-interval floor; presence `.catch`; mandatory release SHA256; device code → debug log). Verified: build clean, 55/55 tests, mock-mode endpoint smoke test, headless client boot. DEFERRED (need human sign-off): xlsx replacement (§10 #2), and the `deleted:true` soft-delete flag + note tombstones (§3 data-model changes, §18). | Brian: "fix all and fable verify" on the audit |
| 2026-07-03 | BK+AI | v1.1.3→**v1.2.0** version bump (root + server + client + shared package.json + lockfiles + §1). Minor, not patch: new restore API option (`conflictStrategy`), a startup guard that can refuse to boot a misconfigured non-loopback install, `multer` major bump (1.x→2.x), and behavioral changes to the updater (git rollback) and backup-task registration (de-elevated). Release still to be PACKAGED + PUBLISHED on Windows (`Package-Release.ps1` → `gh release create v1.2.0` with BOTH the .zip and .sha256 — the updater now REQUIRES the sha256 sidecar). Note: existing kiosks keep the old elevated backup task until `Register-BackupTask.ps1`/the installer is re-run once. | Brian approved cutting the audit fixes as v1.2.0 |
